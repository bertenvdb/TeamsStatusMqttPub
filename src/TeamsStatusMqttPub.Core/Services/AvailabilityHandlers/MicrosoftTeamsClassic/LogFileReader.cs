...
namespace TeamsStatusMqttPub.Core.Services.AvailabilityHandlers.MicrosoftTeamsClassic;
...
public class LogFileReader : ILogFileReader
{
    ...
    public List<string> ReadLinesOfInterest(string logFilePath)
    {
        ...
        while (true)
        {
            ...
            {
                string line = new string(lineBuilder.ToArray()).Trim();

                if (ProcessLineAndStop(line)) return _linesOfInterest;
    ...
    private char[] ReadCharacter(FileStream stream)
    {
        ...
        while (chars.Contains(invalidChar))
        {
            ...
            if (iteration > 4) throw new OverflowException();
            ...
            int charCount = Encoding.UTF8.GetCharCount(buffer, 0, _bufferLength);
    ...
    internal static bool ContainsEventToken(string line)
    {
        return line.Contains(EventDataTokens.BaseToken);
    }
}
