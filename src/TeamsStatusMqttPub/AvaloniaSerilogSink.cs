...
public class AvaloniaSerilogSink : ILogSink
{
    ...
    {
        return _logger.IsEnabled(ToSerilogLogLevel(level));
    }

    public void Log(LogEventLevel level, string area, object? source, string messageTemplate)
    {
        if (InOverrideBelowMinLevel(source, level)) return;
    ...
    {
        return (Serilog.Events.LogEventLevel)level;
    }
    ...
    private bool InOverrideBelowMinLevel(object? source, LogEventLevel level)
    {
        ...
        return !string.IsNullOrEmpty(sourceString) &&
               _overrides.TryGetValue(sourceString, out LogEventLevel minLevel) &&
               level < minLevel;
    }
}
